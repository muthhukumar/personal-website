...
### <Date date="2021-12-29" className="text-md md:text-xl" />

#### Addded 

- Added security header to the html request

---

### <Date date="2021-12-28" className="text-md md:text-xl" />

#### Added

- Added seo stuff(description, robots, pageame, image, apple-mobile-web-app-title) to the changelog page
- Added seo stuff(description, robots, pageame, image, apple-mobile-web-app-title and canonical link) to the about page
- Added seo stuff(description, robots, pageame, image, apple-mobile-web-app-title) to the blog page
- Added seo stuff(canonical link) to the blog post page
- Added seo stuff(description, robots, pageame, image, apple-mobile-web-app-title) to the quotes page
- Added `X-Robots-Tag: no-index` header to the 'rss.xml`and`sitemap.xml` file

#### Changed

- Moved the canonical url from the root links to index route.
- Changed the lastmod late of all the routes in the sitemap file

#### Removed

- Removed `rss.xml` link from the sitemap file
- Removed the meta function from index route
- Removed rickroll routes

---

### <Date date="2021-12-27" className="text-md md:text-xl" />
...
- Now, all the seo stuff are coming from the SEO schema
- Now, Google analytics will not track in development mode
- Updated blog page to use the seo stuff.
- Updated the `sitemap.xml` file to include all the blog links
- Changed the bg color to matt black
- Updated some type stuff.
...
- Removed graphql request package and migrated to fetch. So migrating to cf pages will be easy.
