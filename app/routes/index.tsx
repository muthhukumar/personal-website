export default function Index() {
  return (
    <div className="p-8 pt-0 mt-32 lg:p-16 h-[55vh]">
      <div className="container max-w-5xl mx-auto my-auto mt-12 text-primary">
        <div className="text-xl font-normal">Hi, I&apos;m </div>
        <h1 className="text-5xl font-bold md:text-8xl">Muthukumar</h1>
        <p className="mt-8 text-xl font-normal text-gray-800 dark:text-gray-400">
          I&apos;m a full stack wev developer, who likes to code and try out new technologies.
          Passionate in learning new stuff.
        </p>
      </div>
    </div>
  )
}
